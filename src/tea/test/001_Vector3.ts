...
describe("Vector3", () => {
	...
	it("normalized", () => {
		...
		var n = a.normalized;
		var m = n.magnitude;
		assert(Mathf.approximately(m, 0));
		assert(n.x === 0);
		assert(n.y === 0);
		assert(n.z === 0);
		for (var i = 0; i < 10; i++) {
			...
			n = a.normalized;
			m = n.magnitude;
			assert(Mathf.approximately(m, 1));
	...
	it("approxEquals", () => {
		...
		var b = a.clone();
		assert(a.equals(b));
		b.x += Mathf.Epsilon * 0.9;
		b.y += Mathf.Epsilon * 0.9;
		b.z += Mathf.Epsilon * 0.9;
	...
	it("angle", () => {
		...
		a = new Vector3(1, 0, 0);
		b = new Vector3(1, 1, 0);
		assert(Mathf.approximately(a.angle(b), Math.PI / 4));
		a = new Vector3(0, 1, 0);
		b = new Vector3(1, 1, 0);
		assert(Mathf.approximately(a.angle(b), Math.PI / 4));
		a = new Vector3(0, 0, 1);
		...
		a = new Vector3(1, 0, 0);
		b = new Vector3(1, 0, 0);
		assert(Mathf.approximately(a.angle(b), 0));
		a = new Vector3(0, 1, 0);
		b = new Vector3(1, 0, 0);
		assert(Mathf.approximately(a.angle(b), Math.PI / 2));
		a = new Vector3(1, 0, 0);
		b = new Vector3(0, 1, 0);
		assert(Mathf.approximately(a.angle(b), Math.PI / 2));
		a = new Vector3(1, 0, 0);
		b = new Vector3(-1, 0, 0);
		assert(Mathf.approximately(a.angle(b), Tea.radians(180)));
		a = new Vector3(1, 0, 0);
		b = new Vector3(-1, -1, 0);
		assert(Mathf.approximately(a.angle(b), Tea.radians(135)));
		a = new Vector3(1, 0, 0);
		b = new Vector3(0, -1, 0);
		assert(Mathf.approximately(a.angle(b), Tea.radians(90)));
	});
	...
	it("clampMagnitude", () => {
		for (var i = 0; i < 10; i++) {
			...
			assert(m != null);
			assert(m.magnitude <= 2 + Mathf.Epsilon);
	...
	it("rotateTowards", () => {
		...
		assert(Mathf.approximately(c.magnitude, a.magnitude));
		assert(Mathf.approximately(c.x, a.magnitude));
		assert(Mathf.approximately(c.y, 0));
		assert(Mathf.approximately(c.z, 0));
		c = a.rotateTowards(b, 1, 1);
		assert(Mathf.approximately(c.magnitude, b.magnitude));
		assert(Mathf.approximately(c.x, b.magnitude));
		assert(Mathf.approximately(c.y, 0));
		assert(Mathf.approximately(c.z, 0));
		c = a.rotateTowards(b, 0, 0);
		assert(Mathf.approximately(c.magnitude, a.magnitude));
		assert(Mathf.approximately(c.x, a.x));
		assert(Mathf.approximately(c.y, a.y));
		assert(Mathf.approximately(c.z, a.z));
		c = a.rotateTowards(b, 0, 1);
		assert(Mathf.approximately(c.magnitude, b.magnitude));
		var m = b.magnitude / a.magnitude;
		assert(Mathf.approximately(c.x, m));
		assert(Mathf.approximately(c.y, m));
		assert(Mathf.approximately(c.z, m));
	...
	it("slerp", () => {
		...
		assert(c.z === 2);
		c = a.slerp(b, 1);
		assert(Mathf.approximately(c.x, -2));
		assert(Mathf.approximately(c.y, -2));
		assert(Mathf.approximately(c.z, 0));
		c = a.slerp(b, 2);
		assert(Mathf.approximately(c.x, -2));
		assert(Mathf.approximately(c.y, -2));
	...
	it("slerpUnclamped", () => {
		...
		assert(Mathf.approximately(c.x, 0));
		assert(Mathf.approximately(c.y, 2));
		assert(Mathf.approximately(c.z, -2));
